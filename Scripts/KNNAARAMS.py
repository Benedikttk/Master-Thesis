...
# Define the features (E_final and dE)
X = data[['E_final', 'dE']]

# Set the number of clusters and the random_state for reproducibility
kmeans = KMeans(n_clusters=4, random_state=30, init='k-means++')
...
# Visualize the clusters with labels instead of color bar
plt.figure(figsize=(8, 6))

# Scatter plot with cluster labels as markers
for cluster in range(4):  # Iterate over the 4 clusters
    cluster_data = data[data['Cluster_KMeans'] == cluster]
    plt.scatter(cluster_data['E_final'], cluster_data['dE'], label=f'Cluster {cluster}', s=5)

plt.xlabel('E_final [keV]')
plt.ylabel('dE [keV]')
plt.title('K-Means Clustering of Data')

# Adding the legend with cluster labels
...
# Get silhouette scores for each point
silhouette_vals = silhouette_samples(X, data['Cluster_KMeans'])

# Add silhouette values to the data
data['silhouette_score'] = silhouette_vals

# Focus on the specific cluster (Cluster 1)
cluster_1_data = data[data['Cluster_KMeans'] == 1]

# Calculate the average silhouette score for Cluster 1
cluster_1_silhouette_score = cluster_1_data['silhouette_score'].mean()

print(f"Average Silhouette Score for Cluster 1: {cluster_1_silhouette_score}")

# Higher silhouette score means better-defined and more confident cluster

# Get the centroid of Cluster 1
cluster_1_centroid = kmeans.cluster_centers_[1]

# Calculate the distance of each point in Cluster 1 to the centroid
cluster_1_data = data[data['Cluster_KMeans'] == 1]
cluster_1_data['distance_to_centroid'] = np.linalg.norm(cluster_1_data[['E_final', 'dE']].values - cluster_1_centroid, axis=1)

# Calculate the mean distance to the centroid
mean_distance_to_centroid = cluster_1_data['distance_to_centroid'].mean()

print(f"Mean Distance to Centroid for Cluster 1: {mean_distance_to_centroid}")

# Calculate the distances between Cluster 1's centroid and the other centroids
centroids = kmeans.cluster_centers_

# Get the centroid of Cluster 1
cluster_1_centroid = centroids[1]

# Calculate the distance from Cluster 1's centroid to the others
distances_to_other_centroids = np.linalg.norm(centroids - cluster_1_centroid, axis=1)

# Print the distance to other centroids
print(f"Distance of Cluster 1's centroid to other centroids: {distances_to_other_centroids}")

# Get the number of points in Cluster 1
cluster_1_size = cluster_1_data.shape[0]

print(f"Number of points in Cluster 1: {cluster_1_size}")
...
# Assuming 'data' is your DataFrame with clusters already assigned
# Make sure you have the KMeans clustering and 'Cluster_KMeans' column in 'data'

# Create the figure
fig, ax = plt.subplots(figsize=(8, 6))

# Loop through each cluster and plot the outline and points
for cluster_id in data['Cluster_KMeans'].unique():
    ...
    # Perform Convex Hull to get the boundary points
    points = cluster_points[['E_final', 'dE']].values
    if len(points) >= 3:  # Convex hull requires at least 3 points
        hull = ConvexHull(points)
        hull_points = np.array([points[vertice] for vertice in hull.vertices])
        
        # Plot the convex hull boundary with alpha transparency
        ax.fill(hull_points[:, 0], hull_points[:, 1], color=sns.color_palette("viridis", n_colors=4)[cluster_id], alpha=0.3)
        ax.plot(hull_points[:, 0], hull_points[:, 1], color=sns.color_palette("viridis", n_colors=4)[cluster_id], lw=2)  # Outline the cluster

    # Scatter plot of points in the cluster (with same colors)
    ax.scatter(cluster_points['E_final'], cluster_points['dE'], c=[sns.color_palette("viridis", n_colors=4)[cluster_id]] * len(cluster_points), label=f'Cluster {cluster_id}')
...
# --- Generating Lists of Points for Each Cluster ---
cluster_points_dict = {}

# Loop through each cluster and store the x, y values
for cluster_id in data['Cluster_KMeans'].unique():
    cluster_points = data[data['Cluster_KMeans'] == cluster_id]
    
    # Store the x (E_final) and y (dE) values in a dictionary
    cluster_points_dict[cluster_id] = {
        'x_values': cluster_points['E_final'].tolist(),
        'y_values': cluster_points['dE'].tolist()
    }

# Print the list of points for each cluster
for cluster_id, points in cluster_points_dict.items():
    print(f"Cluster {cluster_id} - X values: {points['x_values']}")
    print(f"Cluster {cluster_id} - Y values: {points['y_values']}")
